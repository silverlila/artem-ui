...
export interface ListBoxProps<T> {
  items: Item<T>[];
  selected: Item<T>;
  setSelected: (state: Item<T>) => void;
}
export function Listbox<T>({ items, selected, setSelected }: ListBoxProps<T>) {
  return (
    <ListBoxUI value={selected} onChange={setSelected}>
      <div className="relative mt-1">
        <ListBoxUI.Button className="border relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
